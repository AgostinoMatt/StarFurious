...
namespace RFLib
{
	public class RFHighScoreRenderer : MonoBehaviour, IRFHighScoreHandler 
	{
		...
		virtual public void EditHighScoreStart(string valid_chars, RFHighScoreUpdateCallback updateCallback)
		{
			...
			currentSelectedCharIndex = selectableCharacters.IndexOf(workingValue.Substring(0,1));
		...
		void Update () 
		{
			if( isEditMode )
			{
				...
				{	
					...
					float CaretAxis = CrossPlatformInputManager.GetAxis( MoveCaretAxis );
					float CharsAxis = CrossPlatformInputManager.GetAxis( RotateCharsAxis );
					bool moveChars = Mathf.Abs( CharsAxis ) > Mathf.Abs( CaretAxis );

					//Mobile Input axis may need changed, depending on input rig setup
					// For now, we want the axis values to be near 1.0f in mobile in order to keep
					// input problems from occurring, such as player barely moving the mobile joystick and
					// characters or caret movement becomes unpredictable
					#if MOBILE_INPUT
					float axisThreshold = 0.95f;
					#else
					float axisThreshold = 0.01f;
					#endif

					// Move Index Right
					if( !moveChars && CaretAxis  > axisThreshold )
					{
						updateCaretIndex( 1 );	
					}
					// Move Index Left
					else if(!moveChars && CaretAxis < -axisThreshold )
					{
						updateCaretIndex( -1 );	
					}
					// Scroll current initials character forward through Selected Characters
					else if(moveChars && CharsAxis  > axisThreshold )
					{
